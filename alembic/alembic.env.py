...
1. `target_metadata` points at `db.models.Base.metadata` — the ORM models
   in `db/models.py` are the single source of truth for schema shape.
   `alembic revision --autogenerate` diffs against those models, not
   against a second hand-maintained schema description.
